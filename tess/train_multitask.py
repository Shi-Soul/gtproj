...
from models.impala_v4_new import Model
...
def main():
    ...
    print("Obs Space:",obs_s)
    print("Act Space: ",act_s)
    ...
    agent:Model = model(obs_s[1:], num_act, task_count ,inv="prisoner" in args.substrate)
    ...
    if load:
        ...
        print("\nLOADED\n")
    print("Num of Net Params",sum([i.numel() for i in agent.parameters()]))
    ...
    for update in tqdm_iter:
        ...
        for step in range(num_steps):
            ...
            # timestep = torch.tensor(env.get_attr("counter"),device="cuda").view(-1,1).expand(-1,act_s[0]).reshape(-1,1) / 1000
            ...
            with torch.no_grad():
                ...
                act, log_prob, value, (h_0, c_0) = agent.sample_act_and_value(batch_o,timestep, history=(h_0,c_0), ready_to_shoot=batch_shoot,\
        ...
        with torch.no_grad():
            ...
            val_plus1 = agent.get_value(batch_o,timestep, history=(h,c), m=config.m, cut_size=config.cut_size, a=config.a, b=config.b, var=config.var, **pri_kwargs).view(num_envs,act_s[0],task_count)
            ...
            for start in range(0, batch_size, minibatch_size):
                ...
                with torch.no_grad():
                    for i in range(mb_obs.shape[0] - 1):
                        if pri_kwargs["inv"]:
                            (h_n,c_n) = agent.lstm_layer(mb_obs[i].view((-1,)+shape),timestep, history=(h__0[-1],c__0[-1]),inv=mb_invs[i].view(-1,2))
                        else: 
                            (h_n,c_n) = agent.lstm_layer(mb_obs[i].view((-1,)+shape),timestep, history=(h__0[-1],c__0[-1]))
                ...
                new_logprob, entropy, value, estimation = agent.check_action_and_value(mb_obs,timestep,\
        ...
        # print("Total Update nums:",(update +1) * batch_size)
        tqdm_iter.set_description(f"Rew: <{mean_real:.4f}|{mean_epis:.4f}>\tVec <{vec_means.numpy()}|{vec_dev.numpy()}|{vec_v.numpy()}>")
