...
def get_config(
    ...
    test_env = sp_utils.env_creator(config.env_config)
...
def train(config, num_iterations=1):
    ...
    if USE_WANDB:
        wandb_project = f"Dev_"+ config.env_config.get("substrate")
        ...
        wdb_callbacks = []
    tune.register_env("meltingpot", sp_utils.env_creator)
    ray.init()
    stop = {
    ...
    return tune.Tuner(
        ...
        run_config=air.RunConfig(stop=stop, callbacks=wdb_callbacks,verbose=1),
    ).fit()
